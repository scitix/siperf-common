...
: "${LOAD_CHECKPOINT:="/data/dataset/ckpt4000-consumed_samples=0"}"  # if set, training starts from this checkpoint (see SHARE_RERUNS effects below). Otherwise starts from scratch.
...
if [ ${SHARE_RERUNS:-0} -eq 1 ]; then
  NEMO_RESULTS_SUBDIR='shared_logs'
else
...
# else
readonly _explicit_log_dir=/data/mlperf_training/result/${NEMO_RESULTS_SUBDIR:-""}
# fi
...
#   EXTRA_ARGS+=" data_prefix@model.data.data_prefix=/data/dataset/preprocessed_c4_spm "
#   EXTRA_ARGS+=" data_prefix@model.data.data_prefix=benchmark_c4"
#   EXTRA_ARGS+=" exp_manager.create_tensorboard_logger=True"
...
${LOGGER:-} ${CMD[@]} /data/mlperf_training/training_results_v4.1/NVIDIA/benchmarks/gpt3/implementations/eos_n32_ngc24.09_nemo/megatron_gpt_pretraining_custom.py \
  $EXTRA_ARGS \
  ; ret_code=$?
